...
KernelRunner2::KernelRunner(uint32_t type, uint32_t version, uint32_t passes,
      ...
      blocksOut(new uint8_t[batchSize * 1 * ARGON2_BLOCK_SIZE])
{
    ...
    CudaException::check(cudaMallocHost(&memory, memorySize));
...
void *KernelRunner2::getInputMemory(size_t jobId) const
{
    size_t copySize = 1 * 2 * ARGON2_BLOCK_SIZE;
    return blocksIn.get() + jobId * copySize;
}
const void *KernelRunner2::getOutputMemory(size_t jobId) const
{
    size_t copySize = 1 * ARGON2_BLOCK_SIZE;
    return blocksOut.get() + jobId * copySize;
}

void KernelRunner2::copyInputBlocks()
{
    ...
    CudaException::check(cudaMemcpy2DAsync(
                             memory, jobSize,
                             blocksIn.get(), copySize,getInputMemory
                             copySize, batchSize, cudaMemcpyHostToDevice,
                             stream));
}

void KernelRunner2::copyOutputBlocks()
{
    size_t jobSize = static_cast<size_t>(lanes) * segmentBlocks
            * ARGON2_SYNC_POINTS * ARGON2_BLOCK_SIZE;
...
void KernelRunner2::runKernelOneshot(uint32_t lanesPerBlock,
                                    size_t jobsPerBlock)
{
...
void KernelRunner2::run(uint32_t lanesPerBlock, size_t jobsPerBlock)
{
...
void KernelRunner2::runHostRam(uint32_t lanesPerBlock, size_t jobsPerBlock)
{
    CudaException::check(cudaEventRecord(start, stream));
