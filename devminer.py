...
required_settings = ['difficulty', 'memory_cost', 'cores', 'devacc', 'last_block_url']
...
account = config['Settings']['devacc']
    ...
    with tqdm(total=None, dynamic_ncols=True, desc=f"{GREEN}Mining{RESET}", unit=f" {GREEN}Blocks{RESET}") as pbar:
        ...
        while True:
            if(not running):
                break
            try:
                BlockDir = f"gpu_found_blocks_dev_tmp/"
...
if __name__ == "__main__":
    ...
    print(f"Dev Mining with: {RED}{account}{RESET}")
    if(gpu_mode):
